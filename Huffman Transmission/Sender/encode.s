...
encode:
    push {fp, lr}
    add fp, sp, #4
    @ use the stack to store strlen(message) (@sp + 12), &hcode[current].letter
    @ (@sp+8), &hcode[current].code[0] (@sp +4), &hcode[current].size (@sp)
    sub sp, sp, #16
    ...
    outerWhileLoop:
        ...
        encoding:
            ...
            @ send huffman data code
            str r0, [sp, #-4]!
            str r1, [sp, #-4]!
            str r2, [sp, #-4]!
            str r6, [sp, #-4]!

            mov r1, sp              @ r1 = address of int packet
            mov r0, #3              @ r0 = socketFp
            mov r2, #4              @ r2 = sizeof(code)
            bl write
